...
export default {
  ...
  methods: {
    ...
    _initCenteredSlider() {
      ...
      const slideWidth = slides[0].offsetWidth + 20; // 80vw + margin-right
    ...
    _centerSlide(index) {
      ...
      this.state.target = offset - containerCenter + this.state.slideWidth / 2;
    ...
    _centerNearestSlide() {
      const containerCenter = this.state.container.offsetWidth / 2;
      const estimatedIndex = Math.round((this.state.current + containerCenter - this.state.slideWidth / 2) / this.state.slideWidth);
