...
export default {
    ...
    _showSection0(direction) {
        ...
        // Optionally, reset gradients or background settings
        this._timelineSection0Show.to(this._postProcessing.passes.finalPass.material.uniforms.uGradient1Strength, { value: 0.2, duration: 2 }, 0);
    },

    _hideSection0(direction) {
        // Optional hide animations (if needed)
    },
    ...
    _showSection1(direction) {
        ...
        this._timelineSection1Show.to(this._cameraAnimation, {
            duration: 2,
            mouseRotationXOffset: 2,     // Slight right rotation
            mouseRotationYOffset: 0.1,   // Small tilt
            zoomZ: 6.5,                   // Zoom in a little
            ease: 'power2.inOut',
        }, 0);

        // Optionally, update background gradients
        this._timelineSection1Show.to(this._postProcessing.passes.finalPass.material.uniforms.uGradient1Strength, { value: 0.3, duration: 2 }, 0);
    },

    _hideSection1(direction) {},

    /**
     * Section 2: Closer dynamic view
     */
    _showSection2(direction) {
        this._timelineSection2Show = gsap.timeline();
        this._timelineSection2Show.to(this._cameraAnimation, {
            duration: 2,
            mouseRotationXOffset: 1,
            mouseRotationYOffset: 0.2,
            zoomZ: 4.5,
            ease: 'power2.inOut',
        }, 0);

        this._timelineSection2Show.to(this._postProcessing.passes.finalPass.material.uniforms.uGradient1Strength, { value: 0.4, duration: 2 }, 0);
    },
